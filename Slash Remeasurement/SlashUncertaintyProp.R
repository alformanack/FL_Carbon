  ...
  for (o in 1:300){
    ...
    for (j in 1:length(plot_density)){ # specify tree per hectare
      ...
      for (i in 2:observed.a){ # specify how long to run the simulation (years)
        ...
        if (Diameter[i,j]>=0) { M<- rbinom(1,1,(initial.par[o,7]*exp(initial.par[o,8]*Diameter[i,j]*2.54)))}
...
save(final_list_slash, file = "C:/Users/Alicia/Documents/GitHub/FL_Carbon/Slash Remeasurement/SlashInitial.rdata")
