...
envdata<-read.csv("slash_onemeterres.csv", header=T, sep=",")
...
for (s in 1:42634) {
  ...
  plot_density<- 2000
  observed.a<- 40
  predict.tasb<-matrix(nrow = 1, ncol = 1,0)
  predict.d<-matrix(nrow = 1, ncol = 1,0)
    ...
    # save average modeled diameter
    predict.d<-mean(Diameter[observed.a,])
    predict.tasb<-sum(TASB[observed.a, ])*.5*1000*(1/10000)
  
  
  # set up dataframe to store simulated data
  df<-cbind(predict.d, observed.a, plot_density, temp, aridity, predict.tasb)
  
  df<-data.frame(df)
  
  colnames(df)<-c("Modeled_Diameter","Age", "Tree_Density", "Temperature", "Aridity", "Modeled_Biomass")
...
write.csv(final_list_slash, "C:/Users/Alicia/Documents/GitHub/FL_Carbon/simulations_onemeterres.csv")
