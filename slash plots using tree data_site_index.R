     ...
     for (i in 1:100){ # specify how long to run the simulation (years)
       ...
       if (!is.na(Diameter[i,j,h])) {
        ...
        # if (Diameter[i,j,h-1]>0 & !is.na(Diameter[i,j,h-1])) {
          prob_denom<-(1+exp(-(-4.2899-0.1022*(Diameter[i,j,h-1]*2.54)+0.2146*(height[i,j,h]/3.281)-
...
# obs_DBH<-exp(rnorm(k,log(observed.d),0.5))
# qqplot(obs_DBH,Diameter[,,observed.a])
