...
age_10cm<-round((10^(-1.632066 + 0.01619077*CN_SCALE - 0.01982971*CN_SCALE*aridity + 1.709267*aridity))*(3.937008^1.748652))

age_20cm<-round((10^(-1.632066 + 0.01619077*CN_SCALE - 0.01982971*CN_SCALE*aridity + 1.709267*aridity))*(7.874016^1.748652))

age_30cm<-round((10^(-1.632066 + 0.01619077*CN_SCALE - 0.01982971*CN_SCALE*aridity + 1.709267*aridity))*(11.81102^1.748652))

age_40cm<-round((10^(-1.632066 + 0.01619077*CN_SCALE - 0.01982971*CN_SCALE*aridity + 1.709267*aridity))*(15.74803^1.748652))

age_50cm<-round((10^(-1.632066 + 0.01619077*CN_SCALE - 0.01982971*CN_SCALE*aridity + 1.709267*aridity))*(19.68504^1.748652))

age_60cm<-round((10^(-1.632066 + 0.01619077*CN_SCALE - 0.01982971*CN_SCALE*aridity + 1.709267*aridity))*(23.62205^1.748652))

age_70cm<-round((10^(-1.632066 + 0.01619077*CN_SCALE - 0.01982971*CN_SCALE*aridity + 1.709267*aridity))*(27.55906^1.748652))

age_80cm<-round((10^(-1.632066 + 0.01619077*CN_SCALE - 0.01982971*CN_SCALE*aridity + 1.709267*aridity))*(31.49606^1.748652))
...
for (o in 1:10){
  ...
  for (j in 1:plot_density){ # specify tree per hectare
    ...
    for (i in 2:observed.a){ # specify how long to run the simulation (years)
      ...
      if (Diameter[i,j]<=3.94){ M<- rbinom(1,1,(.162/4))}
      else if (Diameter[i,j]>3.94 & Diameter[i,j]<=7.97){M<-rbinom(1,1,(.026/8))}
      else if (Diameter[i,j]>7.97 & Diameter[i,j]<=11.81){M<-rbinom(1,1,(.006/13))}
      else if (Diameter[i,j]>11.81 & Diameter[i,j]<=15.75){M<-rbinom(1,1,(.013/16))}
      else if (Diameter[i,j]>15.75 & Diameter[i,j]<=19.69){M<-rbinom(1,1,(.024/20))}
      else if (Diameter[i,j]>19.69 & Diameter[i,j]<=23.62){M<-rbinom(1,1,(.047/22))}
      else if (Diameter[i,j]>23.62 & Diameter[i,j]<=27.56){M<-rbinom(1,1,(.060/133))}
      else if (Diameter[i,j]>27.56){M<-rbinom(1,1,(0.129/57))}
...
# Adjusting cummulative mortality slash pine ------------------------------
temp<-20.30

temp2<-temp^2

age_10cm<-round((10^(2.475538 - 0.3158355*temp + 0.007800543*temp2 + 0.5217529*aridity))*(3.937008^1.728844))

age_20cm<-round((10^(2.475538 - 0.3158355*temp + 0.007800543*temp2 + 0.5217529*aridity))*(7.874016^1.728844))

age_30cm<-round((10^(2.475538 - 0.3158355*temp + 0.007800543*temp2 + 0.5217529*aridity))*(11.81102^1.728844))

age_40cm<-round((10^(2.475538 - 0.3158355*temp + 0.007800543*temp2 + 0.5217529*aridity))*(15.74803^1.728844))

age_50cm<-round((10^(2.475538 - 0.3158355*temp + 0.007800543*temp2 + 0.5217529*aridity))*( 19.68504^1.728844))

age_60cm<-round((10^(2.475538 - 0.3158355*temp + 0.007800543*temp2 + 0.5217529*aridity))*(23.62205^1.728844))

age_70cm<-round((10^(2.475538 - 0.3158355*temp + 0.007800543*temp2 + 0.5217529*aridity))*(27.55906^1.728844))
...
for (o in 1:10){
  ...
  for (j in 1:plot_density){ # specify tree per hectare
    ...
    for (i in 2:observed.a){ # specify how long to run the simulation (years)
      ...
      # Mortality based on diameter class
      if (Diameter[i,j]<=3.94){ M<- rbinom(1,1,(.4/6))}
      else if (Diameter[i,j]>3.94 & Diameter[i,j]<=7.97){M<-rbinom(1,1,(.2/13))}
      else if (Diameter[i,j]>7.97 & Diameter[i,j]<=11.81){M<-rbinom(1,1,(.15/20))}
      else if (Diameter[i,j]>11.81 & Diameter[i,j]<=15.75){M<-rbinom(1,1,(.1/25))}
      else if (Diameter[i,j]>15.75){M<-rbinom(1,1,(0.01/30))}
...
model.1<-lm(data = final_list_slash, log10(Observed_Diameter/Modeled_Diameter)~log10(Age)+Tree_Density+Temperature+Soil_CN*Aridity)
summary(model.1)
# plot(Temperature~Aridity, data= final_list[[1]])
# 
# res.age<-residuals(model.1)
...
model.3<-lm(data = final_list_slash, Observed_Biomass~Modeled_Biomass)
summary(model.3)
...
plot(data = final_list_slash, Observed_Diameter~Modeled_Diameter, xlim = c(0,15), ylim = c(0,15), xlab="Modeled DBH (in)", ylab="Observed DBH (in)",
     main = "Before parameter correction", col.axis="#027368", col="#75BFBF", pch=16, type="p") + abline(0,1, col="#048ABF")
text(Observed_Diameter~Modeled_Diameter, labels=rownames(final_list),data=final_list, cex=0.9, font=2, pos=4)
arrows(final_list$Modeled_Diameter-sdev.dbh, final_list$Observed_Diameter, final_list$Modeled_Diameter+sdev.dbh, final_list$Observed_Diameter, length=0.05, angle=90, code=3)

plot(data = final_list_slash, Observed_Biomass~Modeled_Biomass, xlim = c(0,20000), ylim = c(0,20000), col = "#75BFBF", xlab="Modeled", ylab="Observed", main ="Tree Carbon (gC/m^2)",
     col.axis="#027368", pch=16, type="p") + abline(0,1, col="#048ABF")
